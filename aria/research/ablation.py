...
class ExperimentSpec:
    ...
    n_legs: int = 3                  # 2 → ½ at leg1 + ½ at hold; 3 → thirds at leg1/leg2/hold
    leg1_target: float = 0.05
    leg2_target: float = 0.10
    use_simfin_pead_z: bool = False  # True → cross-sectional PEAD_z from earnings-day returns on SimFin event dates
    top_pct: float = 0.0            # 0 = use runner default (10%); override per-experiment (e.g. 0.05 = top 5%)
    bottom_pct: float = 0.0         # 0 = use runner default (10%); override per-experiment
    notes: str = ""
...
ABLATION_MATRIX: list[ExperimentSpec] = [
    ...
    ExperimentSpec(
        ...
        min_pead_ret=0.01,
        trailing_stop_pct=0.12,
        notes="E33 + 30-day hold; more PEAD drift accumulation → higher avg_win → target RR > 1.5",
    ),
    ExperimentSpec(
        "E38_hold30_trail10",
        signals=["SUE_z"],
        ...
        notes="E37 + tighter 10% trail to reduce avg_loss; combined with 30d hold targets RR > 1.5 + WR > 60%",
    ),
    ExperimentSpec(
        "E39_half_exit_40d",
        signals=["SUE_z"],
        ...
        n_legs=2,
        leg1_target=0.05,
        notes="½ exits at +5% (WR boost); ½ runs 40d with 10% trail (avg_win boost) → targets RR>1.5 + WR>60%",
    ),
    ExperimentSpec(
        "E40_fixed8_30d",
        signals=["SUE_z"],
        ...
        min_pead_ret=0.01,
        stop_loss_pct=0.08,
        notes="Fixed 8% stop on 30d hold; unlike trailing, doesn't fire on profitable trades → WR>60% + RR>1.5",
    ),
    ExperimentSpec(
        "E41_fixed10_30d",
        signals=["SUE_z"],
        ...
        min_pead_ret=0.01,
        stop_loss_pct=0.10,
        notes="Fixed 10% stop on 30d hold; comparison against trailing-10% E38",
    ),
    ExperimentSpec(
        "E42_fixed6_30d",
        signals=["SUE_z"],
        ...
        min_pead_ret=0.01,
        stop_loss_pct=0.06,
        notes="Fixed 6% stop on 30d hold; targeting RR>1.5 while keeping WR>60%",
    ),
    ExperimentSpec(
        "E43_fixed5_30d",
        signals=["SUE_z"],
        ...
        min_pead_ret=0.01,
        stop_loss_pct=0.05,
        notes="Fixed 5% stop on 30d hold; tightest stop that may hit RR>1.5",
    ),
    ExperimentSpec(
        "E44_pead_15yr",
        signals=["PEAD_z"],
        ...
        min_pead_ret=0.01,
        stop_loss_pct=0.06,
        notes="Pure PEAD signal with E42 stop/hold params; no consensus data → active from 2009, targets 15-20% 15yr CAGR",
    ),
    ExperimentSpec(
        "E45_pead_cs_15yr",
        signals=["PEAD_z"],
        weights={"PEAD_z": 1.0},
        use_simfin_pead_z=True,
        hold_days=30,
        bsq_filter=True,
        vol_target=0.15,
        concurrent_vol_adjust=True,
        ...
        min_pead_ret=0.01,
        stop_loss_pct=0.06,
        notes="Cross-sectional PEAD_z from earnings-day reactions on SimFin event dates; no consensus, active 2009+, targets 15-20% 15yr CAGR",
    ),
    ExperimentSpec(
        "E46_pead_cs_highWR",
        signals=["PEAD_z"],
        ...
        top_pct=0.05,
        bottom_pct=0.05,
        notes="E45 + stronger filters: >=3% reaction gate + top/bottom 5% selection → targets WR>60%",
    ),
    ExperimentSpec(
        "E47_pead_esqs_composite",
        signals=["PEAD_z", "ESQS_z"],
        weights={"PEAD_z": 0.6, "ESQS_z": 0.4},
        use_simfin_pead_z=True,
        hold_days=30,
        bsq_filter=True,
        vol_target=0.15,
        ...
        top_pct=0.08,
        bottom_pct=0.08,
        notes="PEAD_z (60%) + ESQS_z (40%) composite; market+fundamental dual-confirmation → targets WR>60%",
    ),
    ExperimentSpec(
        "E48_pead_esqs_tight",
        signals=["PEAD_z", "ESQS_z"],
        ...
        top_pct=0.07,
        bottom_pct=0.07,
        notes="E47 + stricter 2.5% gate + top 7% selection → push WR toward 60%",
    ),
    ExperimentSpec(
        "E49_pead_realistic",
        signals=["PEAD_z"],
        ...
        top_pct=0.05,
        bottom_pct=0.05,
        notes="E46 with vol_target=6% → realistic ~15-20% CAGR at WR>60%",
    ),
]
